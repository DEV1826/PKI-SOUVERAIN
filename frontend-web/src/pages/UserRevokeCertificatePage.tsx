...
export default function UserRevokeCertificatePage() {
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      addToast({ type: 'success', message: 'Demande de revocation envoyee.' });
      setReason('');
    } catch (e: any) {
      addToast({ type: 'error', message: e?.message || 'Impossible de demander la revocation' });
    } finally {
      setBusy(false);
    }
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-body text-[var(--text-muted)] dark:text-neutral-400">
              Demandez la revocation d'un certificat si votre cle a ete compromise.
            </p>
          </div>
          <Button variant="danger" onClick={handleRequest} disabled={busy}>
            {busy ? 'Traitement...' : 'Demander la revocation'}
          </Button>
        </div>
      </div>
